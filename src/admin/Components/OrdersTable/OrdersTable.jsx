...
console.log(serchedData, "{searchOrders");
  ...
  const handleReject = (productId, userId, orderId) => {
    setSingleRejectedData(prevState => ({
      userId: userId, // Set userId
      orderId: orderId, // Set orderId
      rejected_products: [...prevState.rejected_products, productId] // Append productId
    }));
  };
  ...
  const updateTrackingStatus = (value, orderId, userid, Oid,  orderProducts = []) => {
    const rejectedProducts = value === "Reject" 
      ? orderProducts.filter(product => product.order_status === "rejected").map(product => product.product_id) 
      : [];
  ...
  const setRejectAllData = (orderProducts) => {
   ...
   console.log(rejectedProductIds, "rejectedProductIds");
    ...
    setAcceptAndRejectedData((prevData) => ({
      ...prevData,
      rejected_products: rejectedProductIds, 
    }));
  };
      ...
      <div className={`overflow-y-auto ${itemsPerPage > 6 ? 'max-h-[550px]' : ''} border rounded-2xl`}>
        <table className="min-w-full bg-white">
          ...
          <tbody className="whitespace-nowrap">
            {searchOrders.length > 0 ? serchedData.product_items.map((item) =>{
             ...
             return(
              ...
              <td className="p-4">
                ...
                {products.length === 0 ? (
                  ...
                  onChange={(value) => {
                    updateTrackingStatus(value, item.order_details.order_id, item.order_details.userid);
                
                    if (value === "Reject") {
                      setRejectAllData(item.order_products); 
                    }
